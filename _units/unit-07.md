---
title: "Unit 7 — Logistic regression: modeling the boundary directly"
---

**Act II** · composition operation: **condition**

**Sessions:** Fri **Oct 16** · Mon **Oct 19** · Wed **Oct 21**

**PS6** is due at the start of the first session, Fri **Oct 16** · **PS7** (on unit 6's material) is assigned in that same session · due Fri **Oct 23**

> *Is this variant pathogenic?*

## Before the first meeting

Read [chapter 7 — Condition: logistic regression](https://bu-cds-bf550.github.io/bf550-textbook/chapters/ch07-logistic-regression.html). Budget about two hours: the chapter
plus its worked notebook. The chapter's practice problems are ungraded and have worked solutions —
they are the cheapest way to find out whether the reading landed.

## In class

{% include slide-links.html unit="unit-07" %}
{% include lab-links.html unit="unit-07" %}

The topic opens in the first meeting. The rest of the unit is working sessions on the problem set,
with instructors and TAs in the room.


## Where this fits

The [schedule]({{ site.baseurl }}/schedule/) places this unit in the three-act arc, shows what it
composes from, and has the dates and deadlines. The unit format is on the
[problem sets page]({{ site.baseurl }}/assignments/).
