...
Nothing is submitted from a lab. A lab that produces something worth keeping feeds that week's
problem set instead.

| Week | Meeting | Lab |
|---:|---|---|
{% for lab in site.data.labs -%}
| {{ lab.week | remove: "week-" | plus: 0 }} | {{ lab.date }} | [{{ lab.title }}]({{ site.baseurl }}/labs/{{ lab.id }}/) |
{% endfor %}

Each week's labs are also listed on that week's page, under **In class**. The [schedule]({{ site.baseurl }}/schedule/)
