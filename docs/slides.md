...
the unit's reading; they put its ideas to work rather than repeating them.

| Unit | Deck | | |
|---|---|---|---|
{% for deck in site.data.slides -%}
| {{ deck.unit | remove: "unit-" | plus: 0 }} | {{ deck.title }} | [view]({{ site.baseurl }}/slides/{{ deck.file }}.html) | [PDF]({{ site.baseurl }}/slides/{{ deck.file }}.pdf) |
{% endfor %}
